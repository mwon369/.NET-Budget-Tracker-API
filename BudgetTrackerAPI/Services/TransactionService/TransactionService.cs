...
namespace BudgetTrackerAPI.Services.TransactionService
{
    public class TransactionService : ITransactionService
    {
        ...
        public async Task<List<Transaction>?> GetAllTransactionsFilteredByDate(string startDate, string endDate)
        {
            ...
            if (startDate == "")
            {
                return await _context.Transactions.Where(t => t.Date <= DateTime.Parse(endDate)).ToListAsync();
            }

            if (endDate == "")
            {
                return await _context.Transactions.Where(t => t.Date >= DateTime.Parse(startDate)).ToListAsync();
            }
